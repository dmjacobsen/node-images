...
# This includes: ceph, HPE storage hardware, and other open source solutions

function create_ceph_csi_configmap () {
  FSID=$(ceph fsid)
  MON_DUMP=$(ceph mon dump -f json)
   ...
   " > /srv/cray/tmp/csi_configmap.yaml

   kubectl apply -f /srv/cray/tmp/csi_configmap.yaml

}


function create_k8s_ceph_secrets () {
...
function create_sma_ceph_secrets () {
...
function create_cephfs_ceph_secrets () {
...
function create_k8s_storage_class {
  FSID=$(ceph fsid)
  echo "---
  apiVersion: v1
  kind: ConfigMap
...
function create_sma_storage_class {
  FSID=$(ceph fsid)
  echo "---
  apiVersion: v1
  kind: ConfigMap
...
function create_cephfs_storage_class {
  FSID=$(ceph fsid)
  echo "---
  apiVersion: v1
  kind: ConfigMap
